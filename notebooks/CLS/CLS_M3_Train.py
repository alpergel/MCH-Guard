...
def log_optuna_results(study, model_name, output_dir="viz/classification_results/classification_results/optuna"):
    ...
    # csv_path = f"{output_dir}/M3_{model_name}_optuna_results.csv"
...
def visualize_optuna_results(study, model_name, output_dir="viz/classification_results/classification_results/optuna"):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/{model_name}_parallel_coordinates_m3.png")
        plt.close()
        ...
        g.savefig(f"{output_dir}/{model_name}_parameter_relationships_m3.png")
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/{model_name}_learning_curve_m3.png")
        plt.close()
...
def load_and_preprocess_data(filepath="./processed/classification_m3.csv"):
...
def split_and_scale_data(X, y, groups=None):
    ...
    test_csv_path = "./processed/CLS_m3_test.csv"
...
def train_random_forest(X_train, y_train, X_val, y_val, groups=None, perform_hyperparameter_search=False):
    ...
    if perform_hyperparameter_search:
        ...
        n_trials = int(os.environ.get('OPTUNA_N_TRIALS', 250))
    ...
    #         plt.tight_layout()
    #         #plt.savefig('viz/classification_results/rf_feature_importance_m3.png')
    #         plt.close()
...
def evaluate_model(model, X_test, y_test, model_name, optimize_threshold=False):
    ...
    # If requested, optimize the threshold for better specificity
    if optimize_threshold:
        thresholds = np.arange(0.1, 0.9, 0.02)  # Finer granularity in thresholds
        ...
        for threshold in thresholds:
            ...
            # MODIFIED: Adjust weights to heavily prioritize reducing false positives
            # Change from (0.3 * f1) + (0.7 * spec) to more extreme weighting
            score = (0.1 * f1) + (0.9 * spec)
            
            if score > (0.1 * best_f1) + (0.9 * best_spec):
                best_f1 = f1
                best_spec = spec
                best_threshold = threshold
        
        logger.info(f"{model_name} - Optimized threshold: {best_threshold:.2f} (default accuracy: {default_accuracy:.3f})")
...
def plot_roc_curves(results, y_test):
    ...
    plt.tight_layout()
    plt.savefig("viz/classification_results/roc_curves_m3.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_confusion_matrices(results, y_test=None):
    ...
    plt.tight_layout()
    plt.savefig("viz/classification_results/confusion_matrices_m3.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_feature_importance(model, feature_names, model_name, top_n=15, X_train=None, y_train=None):
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        plt.tight_layout()
        plt.savefig(f"viz/classification_results/{model_name.lower()}_feature_importance_m3.png", 
                   dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
        
        logger.info(f"Saved enhanced feature importance plot with confidence intervals to viz/classification_results/{model_name.lower()}_feature_importance_m3.png")
...
def compare_models(results_dict):
    ...
    plt.tight_layout()
    plt.savefig("viz/classification_results/model_comparison_m3.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_learning_curve_curves(estimator, X, y, groups, title='Learning curve', cv_splits=5):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('viz/classification_results/learning_curve_m3.png', dpi=300, bbox_inches='tight')
    plt.close()
    logger.info(f"Saved learning curve to viz/classification_results/learning_curve_m3.png")
    return sizes, train_mean, val_mean


def label_shuffle_baseline(estimator, X, y, groups, cv_splits=5):
    """Compute CV AUC after shuffling labels to check leakage and baseline."""
    cv = GroupKFold(n_splits=cv_splits) if groups is not None else 5
...
# def train_xgboost(X_train, y_train, X_val, y_val, groups=None, perform_hyperparameter_search=False):
#     """Train and evaluate XGBoost model."""
#     logger.info("Training XGBoost model...")
    
#     # Calculate the ratio for scale_pos_weight - increase it to reduce false negatives
#     n_neg = len(y_train) - sum(y_train)
#     n_pos = sum(y_train)
#     # Use a higher weight to penalize false negatives more
#     scale_pos_weight_value = 1.5 * (n_neg / n_pos) if n_pos > 0 else 1.0
    
#     logger.info(f"XGBoost scale_pos_weight: {scale_pos_weight_value:.3f} (higher values reduce false negatives)")
...
#                 # Parameters related to tree-based models
...
#                 # Try different scale_pos_weight values to reduce false negatives
#                 'scale_pos_weight': trial.suggest_float('scale_pos_weight', 
#                                                        scale_pos_weight_value * 0.5, 
#                                                        scale_pos_weight_value * 2.5),
#                 'random_state': 123,
...
#             # Different XGBoost versions handle pruning differently
#             # For older versions, we'll just use eval_set without callbacks
#             try:
#                 # First try with callbacks for newer XGBoost versions
...
#         # Log the optimization results to console and save to file
#         results_df = log_optuna_results(study, "xgboost")
#         logger.info(f"Best trial: {study.best_trial.number}")
...
#         best_xgb.fit(X_train, y_train, eval_set=[(X_val, y_val)], verbose=0)
#     else:
#         # Train base model with default parameters
...
#     y_val_pred_proba = best_xgb.predict_proba(X_val)
#     val_accuracy = accuracy_score(y_val, y_val_pred)
#     val_roc_auc = roc_auc_score(y_val, y_val_pred_proba[:, 1])
...
#     if groups is not None:
#         cv = GroupKFold(n_splits=5)
#         cv_scores = cross_val_score(best_xgb, X_train, y_train, groups=groups, cv=cv, scoring='roc_auc')
#     else:
#         cv_scores = cross_val_score(best_xgb, X_train, y_train, cv=5, scoring='roc_auc')
    
#     # Log results
#     logger.info(f"XGBoost Training Time: {training_time:.2f} seconds")
#     logger.info(f"XGBoost Validation Accuracy: {val_accuracy:.3f}")
#     logger.info(f"XGBoost Validation ROC AUC: {val_roc_auc:.3f}")
#     logger.info(f"XGBoost Mean CV ROC AUC: {cv_scores.mean():.3f} (±{cv_scores.std() * 2:.3f})")
...
def main(hyperparameter_search=True):
    ...
    # # Use raw values for learning_curve API compatibility
    # _ = plot_learning_curve_curves(
    #     estimator_for_curve,
    #     np.asarray(X),
    #     np.asarray(y),
    #     groups,
    #     title='RF Learning Curve (M3)'
    # )
    ...
    joblib.dump(best_rf, "models/cls_m3_model.joblib")
    logger.info("Saved Random Forest model to models/cls_m3_model.joblib")
