...
def log_optuna_results(study, model_name, output_dir="viz/classification_results/optuna"):
    ...
    # csv_path = f"{output_dir}/M2_{model_name}_optuna_results.csv"
...
def visualize_optuna_results(study, model_name, output_dir="viz/classification_results/optuna"):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/{model_name}_parallel_coordinates_m2.png")
        plt.close()
        ...
        g.savefig(f"{output_dir}/{model_name}_parameter_relationships_m2.png")
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(f"{output_dir}/{model_name}_learning_curve_m2.png")
        plt.close()
...
def load_and_preprocess_data(filepath="./processed/classification_m2.csv"):
...
def split_and_scale_data(X, y, groups=None):
    ...
    test_csv_path = "./processed/CLS_m2_test.csv"
...
def train_random_forest(X_train, y_train, X_val, y_val, groups, perform_hyperparameter_search=False):
    ...
    #         plt.tight_layout()
    #         plt.savefig('viz/classification_results/rf_feature_importance_m2.png')
    #         plt.close()
...
def plot_roc_curves(results, y_test):
    ...
    plt.tight_layout()
    plt.savefig("viz/classification_results/roc_curves_m2.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_confusion_matrices(results, y_test=None):
    ...
    plt.tight_layout()
    plt.savefig("viz/classification_results/confusion_matrices_m2.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_feature_importance(model, feature_names, model_name, top_n=15, X_train=None, y_train=None):
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        ax.set_title(f'Top {top_n} Features with Effect Direction - {model_name} (M2 Dataset)', 
        ...
        plt.tight_layout()
        plt.savefig(f"viz/classification_results/{model_name.lower()}_feature_importance_m2.png", 
                   dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
        
        logger.info(f"Saved enhanced feature importance plot with confidence intervals to viz/classification_results/{model_name.lower()}_feature_importance_m2.png")
...
def compare_models(results_dict):
    ...
    plt.tight_layout()
    plt.savefig("viz/classification_results/model_comparison_m2.png", dpi=300, bbox_inches='tight')
    plt.close()
...
def plot_learning_curve_curves(estimator, X, y, groups, title='Learning curve', cv_splits=5):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('viz/classification_results/learning_curve_m2.png', dpi=300, bbox_inches='tight')
    plt.close()
    logger.info("Saved learning curve to viz/classification_results/learning_curve_m2.png")
...
def train_xgboost(X_train, y_train, X_val, y_val, groups=None, perform_hyperparameter_search=False):
    ...
    # Cross-validation scores using GroupKFold to ensure patients stay together
    if groups is not None:
        cv = GroupKFold(n_splits=5)
        cv_scores = cross_val_score(best_xgb, X_train, y_train, groups=groups, cv=cv, scoring='roc_auc')
    else:
        # Fall back to standard cross-validation if groups are not provided
...
def main(hyperparameter_search=True):
    ...
    # Standard evaluation
    results = {}
    # Enable threshold optimization to minimize false negatives
    results['RF'] = evaluate_model(best_rf, X_test, y_test, "Random Forest", optimize_threshold=True)
    ...
    joblib.dump(best_rf, "models/cls_m2_model.joblib")
    logger.info("Saved Random Forest model to models/cls_m2_model.joblib")
