...
# ------------------------------------------------------------------
FILE = "./processed/worsening_m3.csv"      # adjust path
DATECOL = "SCANDATE"
IDCOL   = "RID"
...
# choose covariates
biomarkers   = ["ptau_ab_ratio_csf","PLASMA_NFL", "embedding_scalar","NORM_WMH","NORM_GRAY"]
...
A = A.join(first[biomarkers + demographics])
A = zscore(A, biomarkers + ["PTAGE"]) 
...
num_cols = ["time", "event"] + biomarkers + demographics
...
# ------------------------------------------------------------------
import joblib
joblib.dump(best_cph, 'models/srv_m3_coxph.joblib')
print("Model saved to models/srv_m3_coxph.joblib")

# Save test data for publication figures
A_test.to_csv('processed/SRV_m3_test.csv', index=True)
print("Test data saved to processed/SRV_m3_test.csv")
...
ax.set_title('Feature Effects on Risk with 95% Confidence Intervals - M3 Dataset', 
...
plt.tight_layout()
plt.savefig('viz/survival_results/forest_plot_m3.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
...
ax.set_title('Kaplan-Meier Curves by Risk Group - M3 Dataset (Test Set)')
ax.legend(loc='best')
plt.tight_layout()
plt.savefig('viz/survival_results/kaplan_meier_m3.png', dpi=300)
plt.close()

print("Visualizations saved:")
print("- feature_importance_m3.png")
print("- forest_plot_m3.png")
print("- kaplan_meier_m3.png")
