...
# ------------------------------------------------------------------
FILE = "./processed/worsening_m2.csv"      # adjust path
DATECOL = "SCANDATE"
IDCOL   = "RID"
...
# ------------------------------------------------------------------
import joblib
joblib.dump(best_cph, 'models/srv_m2_coxph.joblib')
print("Model saved to models/srv_m2_coxph.joblib")

# Save test data for publication figures
A_test.to_csv('processed/SRV_m2_test.csv', index=True)
print("Test data saved to processed/SRV_m2_test.csv")
...
ax.set_title('Feature Effects on Risk with 95% Confidence Intervals - M2 Dataset', 
...
plt.tight_layout()
plt.savefig('viz/survival_results/forest_plot_m2.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
...
risk_groups_test = pd.qcut(risk_scores_test, q=3, labels=['Low Risk', 'M2 Risk', 'High Risk'])
...
ax.set_title('Kaplan-Meier Curves by Risk Group - M2 Dataset (Test Set)')
ax.legend(loc='best')
plt.tight_layout()
plt.savefig('viz/survival_results/kaplan_meier_m2.png', dpi=300)
plt.close()

print("Visualizations saved:")
print("- forest_plot_m2.png")
print("- kaplan_meier_m2.png")
