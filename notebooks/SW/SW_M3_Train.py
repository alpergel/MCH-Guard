...
def evaluate_model(model, X_test, y_test, threshold: float | None = None):
    ...
    plt.tight_layout()
    _save_plot("confusion_matrix_large.png")
    plt.close()
    ...
    _save_plot("roc_curve_large.png")
    plt.close()
...
def plot_feature_importance(model, feature_names: list[str], top_n: int = 20):
    ...
    plt.tight_layout()
    _save_plot("feature_importance_large.png")
    plt.close()
...
def main(args: argparse.Namespace | None = None):
    if args is None:
        ...
        parser.add_argument("--data", default="./processed/worsening_large.csv", help="Path to CSV data file")
        parser.add_argument("--hpo", action="store_true", help="Run Optuna hyper-parameter optimisation")
        parser.add_argument("--model-out", default="./models/sw_large_model.joblib", help="Output path for trained model")
    ...
    test_csv_path = "./processed/SW_large_test.csv"
